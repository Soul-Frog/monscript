...
@export var SUCTION_STRENGTH: float = 50 #how quickly the whirlpool pulls player to center at maximum
...
func _process(delta):
	for body in _bodies:
		...
		var force = (1-(dist/_MAX_RADIUS)) * SUCTION_STRENGTH #calculate force based on distance to center
		
		# if within a reasonable range to the center (3 pixels); don't apply force to avoid jittering
		var x_force = 0 if x_dist <= 5 else force * x_direction
		var y_force = 0 if y_dist <= 5 else force * y_direction
